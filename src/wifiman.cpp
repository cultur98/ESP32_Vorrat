...
String ssid = "vorrat-" + String((uint32_t)ESP.getEfuseMac(), HEX);
...
void heartBeatPrint()
{
  ...
  if (WiFi.status() == WL_CONNECTED)
    Serial.println(F("-- CLIENT MODE --"));        // H means connected to WiFi
  else
    Serial.println(F("-- AP MODE --"));        // F means not connected to WiFi
...
bool WiFiManager_loadConfigData()
{
  TRACE1();
  ...
  if (!FileFS.begin(true))
  {
    ...
    Serial.println(F(" FileFS.begin failed (#1)! AutoFormatting."));
  }
...
void WiFiManager_init(char const* host_name, bool force_ap_mode)
{
...
//  ESP_WiFiManager ESP_wifiManager(host_name);
  ESP_WiFiManager ESP_wifiManager(ssid.c_str());
  ESP_wifiManager.setHostname();
  ...
  if ( (Router_SSID != "") && (Router_Pass != "") )
  {
    ...
    ESP_wifiManager.setConfigPortalTimeout(LONG_TIMEOUT_SEC); //If no access point name has been previously entered disable timeout.
    Serial.println(F("Got ESP Self-Stored Credentials. Timeout LONG_TIMEOUT_SEC for Config Portal"));
  }
  else if (WiFiManager_loadConfigData())
  {
    configDataLoaded = true;
    
    ESP_wifiManager.setConfigPortalTimeout(LONG_TIMEOUT_SEC); //If no access point name has been previously entered disable timeout.
    Serial.println(F("Got stored Credentials. Timeout LONG_TIMEOUT_SEC for Config Portal")); 
  }
  else
  {
  ...
  {
    // DRD, disable timeout.
    ESP_wifiManager.setConfigPortalTimeout(LONG_TIMEOUT_SEC);
    
    Serial.println(F("Open Config Portal without Timeout: FORCE mode"));
    initialConfig = true;
  }
